...
export class CheckfilesService {
    async compareOriginalAndChunks(originalFile:string,chunkDir:string,chunkPrefix:string):Promise<boolean>{
        ...
        return  new Promise((resolve,reject)=>{
            exec(`sha256sum input/${originalFile}`, (error, stdout, stderr) => {
                if (error) {
                    console.error(`exec error: ${error}`);
                    reject(false);
                }
                const originalSha256sum = stdout.split(" ")[0];
                exec(`cat ${chunkDir}/${originalFile}.${chunkPrefix}* > ${originalFile}_reconstructed && sha256sum ${originalFile}_reconstructed`, (error, stdout, stderr) => {
                    if (error) {
                        console.error(`exec error: ${error}`);
                        reject(false);
                    }
                   ...
                   const result = originalSha256sum === reconstructedSha256sum;
                    ...
                    exec(`rm ${originalFile}_reconstructed`, (error, stdout, stderr) => {
                        if (error) {
                            console.error(`exec error: ${error}`);
                            reject(false);
                        }
